...
const PairDiscovery = ({ walletAddress, selectedChain = 'ethereum', onTradeRequest = null }) => {
  ...
  // Start/stop discovery based on isDiscoveryActive
  useEffect(() => {
  ...
  const connectWebSocket = () => {
    ...
    try {
      ...
      wsRef.current.onclose = (event) => {
        ...
        if (isDiscoveryActive && reconnectAttempts < maxReconnectAttempts) {
          const delay = Math.min(1000 * Math.pow(2, reconnectAttempts), 10000);
          addToast(`Connection lost. Reconnecting in ${delay/1000}s...`, 'warning');
      ...
      wsRef.current.onerror = (error) => {
        ...
        addToast('Discovery feed error', 'danger');
      };
  ...
  const handleNewPairDiscovered = (eventData) => {
    ...
    if (eventData.risk_score && eventData.risk_score < 30) {
      addToast(`🎯 Low risk pair discovered: ${getTokenSymbol(eventData.token0)}/${getTokenSymbol(eventData.token1)}`, 'success');
    }
  };

  const handleRiskUpdate = (data) => {
    setDiscoveredPairs(prev => prev.map(pair => 
  ...
  const toggleDiscovery = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        addToast(
          isDiscoveryActive ? 'Discovery stopped' : 'Discovery started', 
          'success'
        );
      } else {
  ...
  const updateFilters = (newFilters) => {
    ...
    // Send updated filters to WebSocket
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      wsRef.current.send(JSON.stringify({
        type: 'set_filters',
        filters: newFilters
                ...
                <tr>
                  <td colSpan={8} className="text-center py-4 text-muted">
                    {isDiscoveryActive 
                      ...
                      : 'Start discovery to monitor new pairs'
                    }
                  </td>
                </tr>
              ) : (
              ...
              <Col md={6}>
                ...
                <Table size="sm">
                  <tbody>
                    <tr>
                      <td>Chain:</td>
                      <td><Badge bg="secondary">{selectedPair.chain.toUpperCase()}</Badge></td>
                    </tr>
                    <tr>
                      <td>DEX:</td>
